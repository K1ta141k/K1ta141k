```
 █████╗ ██╗     ██╗
██╔══██╗██║     ██║
███████║██║     ██║
██╔══██║██║     ██║
██║  ██║███████╗██║
╚═╝  ╚═╝╚══════╝╚═╝
```

<div align="center">

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&size=22&duration=3000&pause=1000&color=00FF41&center=true&vCenter=true&width=500&lines=mathematics+%2B+statistics+%40+columbia;incoming+sde+intern+%40+amazon+ads;building+things+that+think)](https://git.io/typing-svg)

</div>

---

### `> whoami`

```bash
ali@columbia:~$ cat /etc/profile
```

```
Name:       Ali Bauyrzhan
Location:   New York, NY
Education:  BA Mathematics-Statistics @ Columbia University
Research:   Research Assistant @ Columbia
Incoming:   Summer SDE Intern @ Amazon (Ads)
```

---

### `> ls ./tech_stack/`

**Languages**

![Python](https://img.shields.io/badge/Python-0d1117?style=flat&logo=python&logoColor=3776AB)
![TypeScript](https://img.shields.io/badge/TypeScript-0d1117?style=flat&logo=typescript&logoColor=3178C6)
![C++](https://img.shields.io/badge/C++-0d1117?style=flat&logo=cplusplus&logoColor=00599C)
![CUDA](https://img.shields.io/badge/CUDA-0d1117?style=flat&logo=nvidia&logoColor=76B900)
![JavaScript](https://img.shields.io/badge/JavaScript-0d1117?style=flat&logo=javascript&logoColor=F7DF1E)
![SQL](https://img.shields.io/badge/SQL-0d1117?style=flat&logo=postgresql&logoColor=4169E1)

**ML / Research**

![PyTorch](https://img.shields.io/badge/PyTorch-0d1117?style=flat&logo=pytorch&logoColor=EE4C2C)
![scikit--learn](https://img.shields.io/badge/scikit--learn-0d1117?style=flat&logo=scikitlearn&logoColor=F7931E)
![HuggingFace](https://img.shields.io/badge/HuggingFace-0d1117?style=flat&logo=huggingface&logoColor=FFD21E)
![Jupyter](https://img.shields.io/badge/Jupyter-0d1117?style=flat&logo=jupyter&logoColor=F37626)

**Infrastructure & DevOps**

![React](https://img.shields.io/badge/React-0d1117?style=flat&logo=react&logoColor=61DAFB)
![FastAPI](https://img.shields.io/badge/FastAPI-0d1117?style=flat&logo=fastapi&logoColor=009688)
![Docker](https://img.shields.io/badge/Docker-0d1117?style=flat&logo=docker&logoColor=2496ED)
![GCP](https://img.shields.io/badge/GCP-0d1117?style=flat&logo=googlecloud&logoColor=4285F4)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-0d1117?style=flat&logo=postgresql&logoColor=4169E1)
![Cloudflare](https://img.shields.io/badge/Cloudflare-0d1117?style=flat&logo=cloudflare&logoColor=F38020)
![Linux](https://img.shields.io/badge/Linux-0d1117?style=flat&logo=linux&logoColor=FCC624)

---

### `> cat ./projects.log`

| Project | Description | Stack |
|---------|-------------|-------|
| [**Pythia**](https://github.com/pythia-trading) | Prediction-market trading system — ML signals, backtesting, alpha validation (bootstrap CIs, deflated Sharpe, PBO), live paper trading | Python, PostgreSQL, ML |
| [**Muse2**](https://github.com/K1ta141k/muse2) | EEG blink detection with Muse 2 BCI headband — signal processing, feature engineering, custom 1D CNN, real-time inference | Python, PyTorch, BrainFlow |
| [**MoltShell**](https://moltshell.sh) | Cloud terminal platform — per-user Linux VMs, WebSocket terminal, tmux sessions, auto-provisioning | TypeScript, Cloudflare Workers, GCP |
| [**Eararchy**](https://github.com/eararchy) | Voice-first AI agent platform — real-time STT/TTS pipeline, microservice architecture, cloud sandboxes | Python, FastAPI, Deepgram, GCP |
| [**Netforce Labs**](https://github.com/netforce-labs) | Multi-agent orchestration — control plane for AI agent swarms, E2B microVM sandboxes, automated PR workflows | TypeScript, PostgreSQL, E2B |

---

### `> cat ./stats.log`

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=K1ta141k&show_icons=true&theme=chartreuse-dark&hide_border=true&bg_color=0d1117&icon_color=00FF41&title_color=00FF41&text_color=c9d1d9" width="48%" />
<img src="https://github-readme-streak-stats.herokuapp.com?user=K1ta141k&theme=chartreuse-dark&hide_border=true&background=0D1117&ring=00FF41&fire=00FF41&currStreakLabel=00FF41" width="48%" />

</div>

---

### `> netstat -contacts`

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0d1117?style=for-the-badge&logo=linkedin&logoColor=0A66C2)](https://www.linkedin.com/in/alibaur/)
[![X](https://img.shields.io/badge/X-0d1117?style=for-the-badge&logo=x&logoColor=white)](https://x.com/aliwka_bb)
[![Email](https://img.shields.io/badge/Email-0d1117?style=for-the-badge&logo=gmail&logoColor=EA4335)](mailto:ali.baurzhan.2004@gmail.com)
[![Portfolio](https://img.shields.io/badge/codren.me-0d1117?style=for-the-badge&logo=googlechrome&logoColor=00FF41)](https://codren.me)

</div>

---

<div align="center">

```
ali@columbia:~$ uptime
up since aug 2020 • building something new right now
```

![Visitor Count](https://komarev.com/ghpvc/?username=K1ta141k&color=00FF41&style=flat-square&label=profile+views)

</div>
